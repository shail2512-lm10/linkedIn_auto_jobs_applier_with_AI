...
class LLMLogger:
    ...
    def __init__(self, llm: ChatOpenAI | ChatHuggingFace):
        self.llm = llm
...
class LLMResumeJobDescription:
    def __init__(self, api_key, strings, openai: bool = False, huggingface: bool = True):
        if openai:
            self.llm_cheap = LoggerChatModel(ChatOpenAI(model_name="gpt-4o-mini", openai_api_key=api_key, temperature=0.8))
            self.llm_embeddings = OpenAIEmbeddings(openai_api_key=api_key)
        elif huggingface:
            hf_llm = HuggingFaceEndpoint(
                repo_id="neuralmagic/Meta-Llama-3.1-8B-Instruct-FP8",
            ...
            self.llm_cheap = LoggerChatModel(ChatHuggingFace(llm=hf_llm))
            self.llm_embeddings = HuggingFaceEmbeddings(
                model_name="BAAI/bge-en-icl", cache_folder="", trust_remote_code=True, model_kwargs=model_kwargs, encode_kwargs=encode_kwargs
            )
